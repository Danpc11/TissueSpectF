...
LFFT_CHROM_LEVELS <- c(as.character(1:22), "X", "Y")
...
load_project_config <- function(path = "config/project.R") {
  ...
  if (length(missing)) lfft_abort("project config missing: ", paste(missing, collapse = ", "))
  cfg$chrom_levels <- cfg$chrom_levels %||% LFFT_CHROM_LEVELS
  cfg
}

load_dataset_config <- function(dataset_id, dir = "config/datasets") {
  path <- file.path(dir, paste0(dataset_id, ".R"))
  if (!file.exists(path)) lfft_abort("No config for dataset ", dataset_id, " at ", path)
  cfg <- source(path, local = TRUE)$value
...
validate_dataset_config <- function(cfg, dataset_id) {
  ...
  missing <- setdiff(required, names(cfg))
  if (length(missing)) {
    lfft_abort("Dataset config ", dataset_id, " missing: ", paste(missing, collapse = ", "))
  }
  if (!identical(cfg$id, dataset_id)) {
    lfft_abort("Config id (", cfg$id, ") does not match filename (", dataset_id, ")")
  }
  if (!is.logical(cfg$has_control_cohort) || is.na(cfg$has_control_cohort)) {
    lfft_abort("has_control_cohort must be TRUE or FALSE in ", dataset_id,
  ...
  if (any(!nzchar(ids)) || anyDuplicated(ids)) {
    lfft_abort("condition_rules need unique, non-empty ids in ", dataset_id)
  }
  cfg$sample_id_column <- cfg$sample_id_column %||% "geo_accession"
  cfg
}

lfft_source_all <- function(dir = "R") {
